...
class Shortcode {
	...
	public function render_shortcode( $attribute, $content = '' ) {
		...
		// $current_weather_html_output = $this->current_weather_html( $weather_data, $custom_name, $show_icon, $show_temperature, $short_description, $show_visibility, $show_sunrise, $show_sunset, $show_pressure, $show_wind, $show_time );
	...
	public function current_weather_data( $data, $time_format, $temperature_scale, $wind_speed_unit, $weather_units, $lw_client_date_format ) {
		...
		if ( '12' === $time_format ) {
			...
			$time_format  = date( $lw_client_date_format, strtotime( $last_update->format( 'Y-m-d g:i:sa' ) ) + $timezone );
			$sunrise_time = date( 'g:i a', strtotime( $sunrise->format( 'Y-m-d g:i:sa' ) ) + $timezone );
			$sunset_time  = date( 'g:i a', strtotime( $sunset->format( 'Y-m-d g:i:sa' ) ) + $timezone );

		} else {
			// 12-hour time to 24-hour time
			$time_format  = date( $lw_client_date_format, strtotime( $last_update->format( 'Y-m-d g:i:sa' ) ) + $timezone );
			$sunrise_time = date( 'H:i', strtotime( $sunrise->format( 'Y-m-d g:i:sa' ) ) + $timezone );
			$sunset_time  = date( 'H:i', strtotime( $sunset->format( 'Y-m-d g:i:sa' ) ) + $timezone );
		}
		return array(
			'city'         => $city,
			'country'      => $country,
	...
	public function current_weather_html( $weather_data, $custom_name, $show_icon, $show_temperature, $short_description, $show_visibility, $show_sunrise, $show_sunset, $show_pressure, $show_wind, $show_time ) {
		...
		$current_temp_icon = $show_icon ? sprintf( '<img src="http://openweathermap.org/img/w/%1$s.png" class="weather-icon" />', $weather_data['icon'] ) : '';
